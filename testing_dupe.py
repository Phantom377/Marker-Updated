...
class MarkerWrapper:
    ...
    def duplicate_spanning_cells(self, markdown: str) -> str:
        """
        Detect rows where only one cell has content and duplicate it across all columns.
        Fixes merged-cell loss from PDF → Markdown.
        """
        ...
        for line in lines:
            ...
            if stripped.startswith("|") and stripped.endswith("|"):
                ...
                # 🔥 If only ONE filled cell → spanning row
                if len(filled) == 1:
                    value = filled[0]
                    new_cells = [value for _ in cells]
        ...
        print("  Spanning row duplication applied")
        return "\n".join(new_lines)
    ...
    def convert_single(
        ...
        if output_format == OutputFormat.MARKDOWN:
            ...
            print("  Fixing merged/spanning table rows...")
            text = self.duplicate_spanning_cells(text)
